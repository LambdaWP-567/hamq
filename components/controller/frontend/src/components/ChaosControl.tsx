...
const ChaosControl: React.FC<Props> = ({ status, onRefresh }) => {
  ...
  const applyConfig = useCallback(async (enabled: boolean) => {
    setLoading(true)
    setError(null)
    const payload: ChaosConfig = { enabled, action, target_namespace: targetNs, dry_run: dryRun }
    try {
      await api.runChaos(payload)
      showToast(enabled ? t('actions.startChaos', 'Chaos started') : t('actions.stopChaos', 'Chaos stopped'), enabled ? 'info' : 'success')
      onRefresh()
    } catch (e) {
  ...
  }, [api, action, targetNs, dryRun, onRefresh, t, showToast])

  const runOnce = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      await api.runChaos({ enabled: true, action, target_namespace: targetNs, dry_run: dryRun })
      showToast(t('actions.runOnce', 'Run Once') + ` — ${action}`, 'info')
      onRefresh()
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Request failed'
      setError(msg)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('chaos.action', 'Chaos Action')}</label>
          <select value={action} onChange={e => setAction(e.target.value as ChaosAction)}
            ...
            {CHAOS_ACTIONS.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('chaos.targetNamespace', 'Target Namespace')}</label>
        ...
        <div>
          ...
          <input type="range" min={10} max={3600} step={10} value={intervalSeconds} onChange={e => setIntervalSeconds(Number(e.target.value))} className="w-full accent-orange-500" />
          <div className="flex justify-between text-xs text-gray-400 mt-0.5"><span>10s</span><span>1h</span></div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('chaos.maxPods', 'Max Pods/Round')}: <span className="font-bold text-orange-500">{maxPods}</span>
          </label>
          <input type="range" min={1} max={5} step={1} value={maxPods} onChange={e => setMaxPods(Number(e.target.value))} className="w-full accent-orange-500" />
      ...
      <label className="flex items-center gap-3 cursor-pointer select-none">
        ...
        <span className="text-sm text-gray-700 dark:text-gray-300">{t('chaos.dryRun', 'Dry Run (log only, no actual changes)')}</span>
      </label>
      ...
      <div className="flex flex-wrap gap-3">
        {!chaosActive ? (
          <button onClick={() => void applyConfig(true)} disabled={loading}
          ...
          <button onClick={() => void applyConfig(false)} disabled={loading}
            ...
            {loading ? t('actions.stopping', 'Stopping…') : t('actions.stopChaos', 'Stop Chaos')}
          </button>
        )}
